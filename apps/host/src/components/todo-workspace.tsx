...
import type { TodoAppEvent, TodoAppHandle } from '@todo-app/contracts';
...
export function TodoWorkspace() {
  ...
  useEffect(() => {
    ...
    void import('@todo-app/mount')
      ...
      .catch((error: unknown) => {
        const message =
          error instanceof Error ? error.message : 'Unknown error while loading injected todo app';
        setLastEvent(`Failed to load injected todo app: ${message}`);
      });
      ...
      <aside className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-400">
            The host imports the todo app directly from the workspace and stays in sync through an
