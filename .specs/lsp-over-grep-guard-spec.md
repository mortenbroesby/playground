...
Harden the existing Astrograph guard into an explicit, token-cost-aware
...
~6,500 tokens consumed. Astrograph returns a precise answer in ~600 tokens.
...
Replace the soft guidance ("Avoid broad shell-based code scans when Astrograph
...
lookups, definition searches, or reference queries. Use Astrograph. Grep costs
...
### 5.2 `code-navigation-guard.mjs` — denial message

Update `buildGuardReason()` to include the token cost:

```
Use Astrograph for code navigation — it costs ~600 tokens vs ~6,500 for
...
2. The denial message from `code-navigation-guard.mjs` includes the ~600 vs ~6,500
   token comparison.
