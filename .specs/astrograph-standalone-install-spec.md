...
- publishable npm package name must be `astrograph`
...
4. The installer must:
   ...
   - prefer `npx astrograph mcp` as the portable execution path
...
- `pnpm --filter astrograph type-lint`
- `pnpm --filter astrograph test -- --run tests/engine-contract.test.ts tests/engine-behavior.test.ts tests/interface.test.ts`
- `pnpm --filter astrograph build:observability`
