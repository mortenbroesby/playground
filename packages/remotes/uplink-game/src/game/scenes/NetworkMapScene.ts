...
export class NetworkMapScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.drawGrid();       // depth 0
    this.initRain();       // depth 1
    this.drawConnections();// depth 2-3
    this.drawNodes();      // depth 4-6
    this.drawUI();         // depth 8-9
    this.drawScanLine();   // depth 10
  }
  ...
  private initRain(): void {
    ...
    this.time.addEvent({
      ...
      repeat: -1,
      callback: () => {
        if (this.rainDrops.length < 22) this.spawnRainDrop(false);
      },
    });
  }
  ...
  private drawConnections(): void {
    ...
    // Dim glow pass (slightly wider, very transparent)
    ...
    // Data packets (small glowing dots)
  ...
  private setupTargetNode(node: NodeDef): void {
    // Pulsing ring via Container so scale tweens center correctly
    ...
    // Blinking callout arrow
    const callout = this.add.text(node.x + node.radius + 14, node.y - 6, '▶ CLICK TO HACK', {
      fontFamily: 'monospace',
      fontSize: '11px',
      color: '#ff6060',
    ...
    zone.on('pointerdown', () => this.scene.start('HackScene', { targetName: 'DARKNET-07' }));
  ...
  private drawUI(): void {
    ...
    const instr = this.add.text(450, 542, '> LOCATE DARKNET-07 (RED NODE) — CLICK TO INITIATE HACK', {
      fontFamily: 'monospace',
      fontSize: '11px',
      color: '#53d1ff',
    }).setOrigin(0.5, 0.5).setDepth(9);
